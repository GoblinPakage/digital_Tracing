...
def func_cookie():
    with app.app_context():
        ...
        logging.debug(res.cookie)
        logging.debug(res2.text)
        print("google cookie",logging.error(res.cookie))
        print("ganalytic text",logging.error(res2.text))
        ...
        return prefix_google + render_template('cookie.html', value=res.cookie, analytic=res2.text, event= data.get('rows')[0][0], visit=data.get('rows')[0][1] )
